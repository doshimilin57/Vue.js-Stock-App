<template>
  <div class="page-container">
    <md-toolbar class="md-primary st-toolbar" md-elevation="1">
      <h3 class="md-title" style="flex: 1">{{$t('dashboard')}}</h3>
      <md-avatar>
        <img src="https://vuematerial.io/assets/examples/avatar.jpg" alt="Avatar">
      </md-avatar>
      <md-button class="md-primary" @click="logout">{{$t('logoutTxt')}}</md-button>
    </md-toolbar>
    <form class="md-layout" @submit.prevent="getData" autocomplete="off">
      <md-card class="md-layout-item md-size-50 md-small-size-100 search">
        <md-card-content>
          <div class="md-layout md-gutter">
            <div class="md-layout-item md-small-size-100">
              <md-autocomplete v-model="symbol" :md-options="symbols" placeholder="Ex. AAPL" aria-labelledby="lblSymbol">
                <label for="lblSymbol" id="lblSymbol">{{$t('inputSymbol')}}</label>
              </md-autocomplete>
            </div>
          </div>
        </md-card-content>
        <div class="loading-overlay" v-if="sending">
          <md-progress-spinner md-mode="indeterminate" :md-stroke="2"></md-progress-spinner>
        </div>
        <md-card-actions>
          <md-button type="submit" class="md-primary st-btn" :disabled="sending">{{$t('searchBtn')}}</md-button>
        </md-card-actions>
      </md-card>
    </form>
    <md-card class="md-layout-item md-size-100 md-small-size-100 chart">
      <highcharts class="stock" :constructor-type="'stockChart'" :options="stockOptions" :updateArgs="updateArgs"  ref="chart"></highcharts>
    </md-card>
    <md-snackbar :md-active.sync="apiError">{{errorMsg}}</md-snackbar>
  </div>
</template>

<script>
import DataService from '../services/data.service'
import symbolsJson from '../json/symbols.json'

export default {
  name: 'Dashboard',
  data () {
    return {
      symbol: '',
      symbols: [],
      symbolsJson: symbolsJson,
      sending: false,
      apiError: false,
      errorMsg: '',
      points: [],
      volumes: [],
      title: '',
      updateArgs: [true, false, true],
      loading: false,
      stockOptions: {
        rangeSelector: {
          enabled: true,
          labelStyle: {
            display: 'none'
          }
        },
        title: {
          text: this.title
        },
        yAxis: [{
          labels: {
            align: 'right',
            x: -3
          },
          title: {
            text: 'OHLC'
          },
          height: '60%',
          lineWidth: 2,
          resize: {
            enabled: true
          }
        }, {
          labels: {
            align: 'right',
            x: -3
          },
          title: {
            text: 'Volume'
          },
          top: '65%',
          height: '35%',
          offset: 0,
          lineWidth: 2
        }],
        tooltip: {
          split: true
        },
        series: [{
          type: 'candlestick',
          name: this.title,
          data: [],
          dataGrouping: {
            units: [[
              'week',
              [1]
            ], [
              'month',
              [1, 2, 3, 4, 6]
            ]]
          }
        }, {
          type: 'column',
          name: 'Volume',
          data: [],
          yAxis: 1,
          dataGrouping: {
            units: [[
              'week',
              [1]
            ], [
              'month',
              [1, 2, 3, 4, 6]
            ]]
          }
        }]
      }
    }
  },
  methods: {
    getData () {
      this.sending = true
      this.apiData(this.symbol)
    },
    apiData (symbol) {
      DataService.getData(symbol).then(
        response => {
          this.title = symbol
          this.sending = false
          if (response.data['Note'] || response.data['Error Message']) {
            this.apiError = true
            this.errorMsg = response.data['Note'] || response.data['Error Message']
          }
          this.setChartData(response)
        },
        error => {
          console.log(error)
        }
      )
    },
    setChartData (response) {
      const ohlc = []
      const volume = []
      const data = response.data
      const dataSeries = data['Time Series (Daily)']
      let temp = []
      for (let row in dataSeries) {
        temp.push(row)
      }
      for (let index = 0; index < temp.sort().length; index++) {
        let row = temp[index]
        ohlc.push([
          new Date(row).getTime(),
          parseFloat(dataSeries[row]['1. open']),
          parseFloat(dataSeries[row]['2. high']),
          parseFloat(dataSeries[row]['3. low']),
          parseFloat(dataSeries[row]['4. close'])
        ])
        volume.push([
          new Date(row).getTime(),
          parseInt(dataSeries[row]['5. volume'])
        ])
      }
      this.points = ohlc
      this.volumes = volume
    },
    logout () {
      setTimeout(() => {
        localStorage.removeItem('user')
        this.$router.push('/')
      }, 300)
    }
  },
  created () {
    this.apiData('AAPL')
    for (const iterator of this.symbolsJson) {
      this.symbols.push(iterator.symbol)
    }
  },
  mounted () {
    const loggedIn = localStorage.getItem('user')
    if (!loggedIn) {
      this.$router.push('/')
    }
  },
  watch: {
    title (newValue) {
      this.stockOptions.title.text = newValue
    },
    points (newValue) {
      this.stockOptions.series[0].data = newValue
    },
    volumes (newValue) {
      this.stockOptions.series[1].data = newValue
    }
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
.page-container {
  align-items: center;
  justify-content: center;
  background: url(data:image/svg+xml;base64,);
  position: absolute;
  height: 100%;
  width: 100%;
  top: 0;
  bottom: 0;
  right: 0;
  left: 0;
  z-index: 0;
}
.page-container {
  text-align: center;
}
.page-container .loading-overlay {
  z-index: 10;
  top: 0;
  left: 0;
  right: 0;
  position: absolute;
  width: 100%;
  height: 100%;
  background: rgba(255, 255, 255, 0.9);
  display: flex;
  align-items: center;
  justify-content: center;
}
.search {
  margin: auto;
  margin-bottom: 20px;
  margin-top: 20px;
}
.stock {
  height: 600px;
}
.md-title {
  text-align: left;
}
</style>
